...
import { RotateCcw, MessageCircle, CheckCircle2 } from 'lucide-react'
...
export default function HeroJourneyIllustration() {
  ...
  return (
    <div ref={sectionRef} className="relative mt-14 md:mt-20">
      <Container>
        <div className="relative overflow-hidden rounded-[20px] border border-line bg-gradient-to-b from-surface to-surface-2/40 shadow-lifted">
          {/* Dotted grid texture */}
          ...
          <div className="relative flex items-center justify-between px-5 py-4 md:px-7 md:py-5">
            <div className="flex items-center gap-2.5">
              <div className={`h-2 w-2 rounded-full transition-colors duration-300 ${allDone ? 'bg-success' : isProcessing ? 'bg-primary' : 'bg-ink-muted'}`}>
                {isProcessing && (
                  <motion.div
            ...
            <button
              onClick={replay}
              className="flex h-9 items-center gap-1.5 rounded-lg border border-line bg-surface/90 px-3 text-[12px] font-medium text-ink-secondary shadow-card backdrop-blur-sm transition-colors hover:text-ink hover:border-line-strong"
          ...
          {/* Main composition */}
          <div className="relative px-5 pb-6 md:px-7 md:pb-8 lg:px-10 lg:pb-10">
            <div className="grid grid-cols-1 items-stretch gap-6 lg:grid-cols-[1fr_1.3fr_1fr] lg:gap-4">
                ...
                >
                  <div className="rounded-xl border border-line bg-surface p-5 shadow-float">
                    <div className="mb-3 flex items-center gap-2.5">
                      ...
                      <div>
                        <p className="text-[14px] font-semibold text-ink">TikTok</p>
                        <span className="text-[12px] text-ink-muted">Comment · just now</span>
                      </div>
                      {step >= 1 && !allDone && (
                        <span className="ml-auto flex items-center gap-1.5 rounded-md bg-primary/10 px-2 py-1 text-[11px] font-medium text-primary">
                    ...
                    <div className="mt-3 flex items-center gap-2 border-t border-line pt-3">
                      <div className="flex h-7 w-7 items-center justify-center rounded-full bg-violet-surface">
                        <span className="text-[10px] font-semibold text-primary">AC</span>
                      </div>
                      <span className="text-[13px] font-medium text-ink-secondary">Alex Chen</span>
                    </div>
                  </div>
                </motion.div>
              </div>
              ...
              <div className="flex flex-col items-center justify-center gap-6 py-4">
                ...
                <div className="relative flex items-center justify-center">
                  {/* Horizontal orbit path through core */}
                  ...
                  <OrbitCore
                    ...
                    active={isProcessing}
                    complete={allDone}
                    label={allDone ? 'Journey complete' : isProcessing ? 'Base360 processing' : 'Base360'}
                ...
                <AnimatePresence mode="wait">
                  {currentStepData && (
                    <motion.div
                      ...
                      className="w-full max-w-[340px] rounded-lg border border-line bg-surface/80 p-3.5 shadow-card backdrop-blur-sm"
                    >
                      <p className="text-[13px] font-semibold text-ink">{currentStepData.label}</p>
                      <div className="mt-2 space-y-1.5">
                        <div className="flex items-start gap-2">
                          <span className="mt-0.5 shrink-0 text-[10px] font-semibold uppercase tracking-wider text-ink-muted">Customer</span>
                          <span className="text-[12px] text-ink-secondary">{currentStepData.customerAction}</span>
                        </div>
                        <div className="flex items-start gap-2">
                          <span className="mt-0.5 shrink-0 text-[10px] font-semibold uppercase tracking-wider text-primary">Base360</span>
                          <span className="text-[12px] text-ink-secondary">{currentStepData.base360Action}</span>
                        </div>
                        <div className="flex items-start gap-2">
                          <span className="mt-0.5 shrink-0 text-[10px] font-semibold uppercase tracking-wider text-success">Record</span>
                          <span className="text-[12px] font-medium text-ink">{currentStepData.recordUpdate}</span>
                        </div>
                      </div>
                    </motion.div>
                  )}
                    ...
                    return (
                      ...
                      >
                        <Icon className={`h-3.5 w-3.5 shrink-0 ${
                          nodeStatus === 'active' ? 'text-primary' : nodeStatus === 'done' ? 'text-success' : 'text-ink-muted'
                        }`} />
                        <span className={`text-[12px] font-medium ${
                          nodeStatus === 'inactive' ? 'text-ink-muted' : 'text-ink'
                        }`}>{node.label}</span>
                        {nodeStatus === 'done' && <CheckCircle2 className="ml-auto h-3 w-3 text-success" />}
                      </motion.div>
                    )
                  })}
                </div>
              ...
              {/* ── Right: Customer record (evolving) ── */}
              <div className="flex items-center justify-center">
                <motion.div
            ...
            <div className="flex flex-col items-center gap-3 lg:hidden">
              <div className="flex items-center gap-1.5 py-1">
                <div className={`h-8 w-px transition-colors duration-500 ${step >= 2 ? 'bg-primary/40' : 'bg-line'}`} />
              </div>
            </div>
          </div>
        </div>
  ...
  return (
    <div className={`rounded-xl border bg-surface p-5 shadow-float transition-colors duration-500 ${allDone ? 'border-success/30' : 'border-line'}`}>
      ...
      <div className="mb-4 flex items-center gap-2.5">
        <div className={`flex h-10 w-10 items-center justify-center rounded-full transition-colors duration-500 ${allDone ? 'bg-success/15' : 'bg-violet-surface'}`}>
          <span className={`text-[13px] font-semibold transition-colors duration-500 ${allDone ? 'text-success' : 'text-primary'}`}>AC</span>
        </div>
        <div>
          <p className="text-[15px] font-semibold text-ink">Alex Chen</p>
          <span className="text-[12px] text-ink-muted">{allDone ? 'Customer' : leadStage}</span>
        </div>
        <span className={`ml-auto rounded-md px-2 py-0.5 text-[11px] font-medium transition-colors duration-500 ${
          allDone ? 'bg-success/10 text-success' : 'bg-violet-surface text-primary'
        }`}>
          {leadStage}
        </span>
      </div>
      ...
      <AnimatePresence>
        {allDone && (
          ...
          >
            ...
            <span className="text-[12px] font-medium text-success">Customer created · Won</span>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
  ...
  return (
    <div className="flex items-center justify-between">
      ...
      <span className={`text-[13px] font-medium transition-colors duration-300 ${
        accent === 'violet' ? 'text-primary' : accent === 'success' ? 'text-success' : 'text-ink'
      }`}>
        {value}
      </span>
    </div>
